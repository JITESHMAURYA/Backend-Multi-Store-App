...
authRouter.post("/api/signup", async (req, res) => {
  try {
    ...
    //check if the email already exists in the Vendors collections
    const existingVendorEmail = await Vendor.findOne({ email });
    if (existingVendorEmail) {
      return res.status(400).json({ msg: "Account already owned by a vendor" });
    }
    ...
    if (existingEmail) {
      ...
      //set otp in temporary store with email as key
...
authRouter.post("/api/signin", async (req, res) => {
  try {
    ...
    if (!findUser.isVerified) {
      return res.status(403).json({ msg: "Email not verified. Please verify email to sign in." })
    }
    else {
